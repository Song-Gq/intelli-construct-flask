import json
from alg.action.convert import driver_action_start_recog
...
def driver_action_recog(json_path, proc_id):
    file_num = 1
    fstatus_dict[proc_id] = False
    fnum_dict[proc_id] = file_num
    fdone_dict[proc_id] = 0
    try:
        res = driver_action_start_recog(json_path, proc_id)
        ...
        print("driver_action_recog(): {}".format(e))
        fnum_dict.pop(proc_id)
        fdone_dict.pop(proc_id)
        fstatus_dict.pop(proc_id)
        return None
